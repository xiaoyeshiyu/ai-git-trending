## 今日热点：Agent 技能体系与开发者工具链升温
今天的 GitHub 热点明显围绕 AI 编程 Agent 的能力工程展开，从 skills、harness、meta-harness、记忆、安全、研究优先开发，到 Claude Code、Codex、OpenCode、Cursor、Hermes 等生态的工具包与工作流扩展，显示开发者正在把 Agent 从单点助手推进到可复用、可编排、可优化的工程体系；同时，榜单也覆盖了本地模型推理服务、开源编码 Agent、格式化库、Node 版本管理、免注册浏览器工具、文本人性化处理、编辑型图表设计，以及公开漏洞 PoC 与安全研究归档等方向，呈现出 AI 原生开发基础设施与传统开发效率工具并行走热的趋势。具体项目摘要如下：
