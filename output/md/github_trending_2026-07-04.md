## 今日热点：AI 编码代理走向工具化与本地化
今日热门项目集中体现了 AI 编码代理生态的快速扩张：从 Codex、Claude Code、MCP、Agent Skills 到终端多代理工作流，开发者正在把大模型能力嵌入代码审查、浏览器调试、Unity 编辑器、.NET 工程和日常研发流程；同时，AI 应用也在向更具体的生产场景落地，覆盖网页 GUI 自动控制、开源渗透测试、本地会议转写总结、机器学习系统学习资料，以及自托管照片、ROM、音乐播放器等个人数据管理工具；系统提示词泄露与技能库项目的走热，也说明围绕模型行为、可复用能力封装和跨工具协作的关注度正在升高。具体项目摘要如下：
