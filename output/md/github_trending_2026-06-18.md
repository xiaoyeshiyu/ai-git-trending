## 今日热点：Agentic 工程与本地智能基础设施升温
今天的 GitHub 热点集中在从 AI 模型到开发者工具、从本地化基础设施到开源生产力平台的全链路升级：Google 的时间序列基础模型、GLM-5、LTX-2 等体现了基础模型继续向预测、多模态与智能工程演进；iroh、zvec、codebase-memory-mcp、flue、KiloCode 等项目则显示 Rust 网络栈、向量数据库、代码知识图谱、沙箱代理框架和编码 Agent 平台正在成为新一代开发基础设施；同时 freeCodeCamp、awesome-artificial-intelligence、ASP.NET Core、Insomnia、Plane、LibreTranslate 和 Android Debloater 等成熟开源项目覆盖教育、Web 框架、API 调试、项目管理、机器翻译与终端设备隐私优化，说明开源生态正在围绕 AI 原生开发、可自托管工具链和高效工程协作持续扩张。具体项目摘要如下：
