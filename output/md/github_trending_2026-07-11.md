## 今日热点：AI 开发工具链与基础设施工程并进
今日热门项目呈现出 AI 编程协作、现代应用框架、测试与自动化、基础设施即代码、C++/Rust 底层能力和智能家居等方向同步升温的态势：Claude Code 模板、Stitch Skills、MCP 服务器、OpenAI Plugins 与 Claude Cookbooks 反映出代理式开发生态快速扩展，Terraform、Ansible、GitHub Actions、Home Assistant 与 PowerToys 延续了工程效率和系统管理需求，Next.js、Nuxt、Bun、Prisma、ASP.NET Core、Cypress 等项目覆盖 Web 全栈与测试链路，Catch2、Abseil、Asio、meshoptimizer、pgrust、F Prime 则显示底层库、数据库与嵌入式软件依然保持高关注度。具体项目摘要如下：
