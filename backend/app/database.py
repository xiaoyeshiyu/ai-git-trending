            ...
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS summarized_projects (
                        ...
                        tech_domain TEXT, analysis TEXT
                    )""")
                ...
                # 迁移：添加 analysis 列（如果不存在）
                try:
                    cursor.execute("SELECT analysis FROM summarized_projects LIMIT 1")
                except sqlite3.OperationalError:
                    cursor.execute("ALTER TABLE summarized_projects ADD COLUMN analysis TEXT")
                    conn.commit()
                    logger.info("✅ Migration: Added analysis column")
