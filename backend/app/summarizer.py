...
from .github_api import get_entity_details, get_entity_repos, get_recent_commits
...
def extract_tech_domain(content: str, project: dict = None) -> str:
    """从LLM输出中提取技术领域"""
    if not content:
        return infer_domain_from_language(project)

    # 匹配 **核心领域**：xxx 格式
    match = re.search(r'\*\*核心领域\*\*[:：]\s*(.+)', content)
    if match:
        domain_text = match.group(1).strip()
        domain = normalize_tech_domain(domain_text)
        if domain != 'Other':
            return domain

    # 对完整内容做关键词匹配
    domain = normalize_tech_domain(content[:2000])
    if domain != 'Other':
        return domain
...
def infer_domain_from_language(project: dict = None) -> str:
    """根据项目语言推断技术领域"""
    if not project:
        return 'Other'
...
def _format_commits(commits):
    """Format recent commits into a readable text block for the LLM prompt."""
    if not commits:
        return ""
    lines = ["近期 Git 提交记录（最近 {} 条）：".format(len(commits))]
    for c in commits:
        lines.append(f"  - [{c['date']}] {c['message']} (by {c['author']})")
    return "\n".join(lines)
...
- **近期动向与发展方向**：[基于最近的 git commit 历史（如果提供了），分析项目近期的开发重点、活跃度和演进方向。例如：最近在修 bug 还是加新功能、是否有重大重构、社区贡献者是否活跃。如果没有 commit 数据，写"暂无 commit 数据可用"。]
...
def get_summary_for_single_project(project):
    ...
    if 'recent_commits' not in project:
        project['recent_commits'] = get_recent_commits(project['name'])
        if project['recent_commits']:
            logger.info(f"  Fetched {len(project['recent_commits'])} recent commits for {project['name']}")
