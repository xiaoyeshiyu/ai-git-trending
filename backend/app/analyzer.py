...
def analyze_trends(days=7):
    ...
    with db._get_connection() as conn:
        ...
        sql_tech_domains = """
        SELECT sp.tech_domain, COUNT(DISTINCT f.project_id) as count
        FROM fact_trending_snapshots f
        JOIN dim_projects p ON f.project_id = p.project_id
        JOIN dim_dates d ON f.date_id = d.date_id
        JOIN summarized_projects sp ON p.name = sp.name
        WHERE d.full_date BETWEEN ? AND ?
          AND sp.tech_domain IS NOT NULL
          AND sp.tech_domain != ''
        GROUP BY sp.tech_domain
        ORDER BY count DESC, sp.tech_domain ASC
        LIMIT 5
        """
        cursor.execute(sql_tech_domains, (start_date.isoformat(), end_date.isoformat()))
        tech_domain_rows = cursor.fetchall()
        ...
        WITH project_dates AS (
            SELECT
                ...
                COUNT(f.snapshot_id) as snapshot_count
            FROM fact_trending_snapshots f
            JOIN dim_dates d ON f.date_id = d.date_id
            WHERE d.full_date BETWEEN ? AND ?
        ...
        SELECT
            ...
            p.description,
            l.name as language,
            (last_snapshot.stars - first_snapshot.stars) as star_increase,
            first_snapshot.stars as start_stars,
            last_snapshot.stars as end_stars
        FROM project_dates pd
        JOIN dim_projects p ON pd.project_id = p.project_id
        LEFT JOIN dim_languages l ON p.language_id = l.language_id
        JOIN dim_dates first_date ON first_date.full_date = pd.first_day
        JOIN dim_dates last_date ON last_date.full_date = pd.last_day
        JOIN fact_trending_snapshots first_snapshot
            ON first_snapshot.project_id = pd.project_id
           AND first_snapshot.date_id = first_date.date_id
        JOIN fact_trending_snapshots last_snapshot
            ON last_snapshot.project_id = pd.project_id
           AND last_snapshot.date_id = last_date.date_id
        WHERE star_increase > 50
        ORDER BY star_increase DESC
        LIMIT 10
        """

        cursor.execute(sql_surging, (start_date.isoformat(), end_date.isoformat()))
    ...
    # 生成技术领域数据，优先使用 LLM 分析后持久化的 tech_domain。
    tech_domains = []
    total_domain_count = sum(row[1] for row in tech_domain_rows) if tech_domain_rows else 0
    for domain, count in tech_domain_rows:
        tech_domains.append({
            "name": domain,
            "count": count,
            "percentage": round((count / total_domain_count) * 100) if total_domain_count else 0
        })
